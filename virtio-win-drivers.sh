...
virtio_release="virtio-win-0.1.189_amd64"
virtio_release_folder="virtio-win-0.1.189-1"
...
if [ "$have_tools" = true ]; then
	...
	wget -c "${virtio_uri}" -O ".${virtio_release}.vfd" && 7z x -ofloppy/drivers/"${virtio_release}" ".${virtio_release}.vfd" amd64
    fi
fi
